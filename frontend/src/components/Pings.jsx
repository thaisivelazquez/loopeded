    ...
    <div className="looped-page" style={{ maxWidth: 640 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 9, marginTop: 20 }}>
        {pings.list.map(ping => (
          <div
            key={ping.key}
            onClick={ping.open || undefined}
            style={{ display: 'flex', alignItems: 'center', gap: 13, background: ping.bg, border: '1px solid rgba(255,255,255,.75)', backdropFilter: 'blur(12px)', borderRadius: 16, padding: '14px 16px', cursor: ping.open ? 'pointer' : 'default' }}
          >
            ...
            {ping.isFriendRequest && (
              <div style={{ marginLeft: 'auto', display: 'flex', gap: 6, flex: 'none' }}>
                <button onClick={(e) => { e.stopPropagation(); ping.decline(); }} style={{ cursor: 'pointer', border: 'none', background: 'rgba(58,44,40,.08)', color: 'rgba(58,44,40,.7)', font: '800 11.5px Nunito,sans-serif', padding: '8px 14px', borderRadius: 999 }}>decline</button>
                <button onClick={(e) => { e.stopPropagation(); ping.accept(); }} style={{ cursor: 'pointer', border: 'none', background: accent, color: '#fff', font: '800 11.5px Nunito,sans-serif', padding: '8px 14px', borderRadius: 999 }}>accept</button>
              </div>
            )}
            {ping.hasAction && (
              <button onClick={(e) => { e.stopPropagation(); ping.act(); }} style={{ marginLeft: 'auto', cursor: 'pointer', border: 'none', background: accent, color: '#fff', font: '800 11.5px Nunito,sans-serif', padding: '8px 14px', borderRadius: 999, flex: 'none' }}>{ping.actionLabel}</button>
            )}
            {ping.going && (
            ...
            <button
              onClick={(e) => { e.stopPropagation(); ping.del(e); }}
