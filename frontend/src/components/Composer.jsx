    ...
    <div onClick={composer.close} className="looped-modal-overlay" style={{ background: 'rgba(58,44,40,.25)', backdropFilter: 'blur(3px)', zIndex: 55 }}>
      <div onClick={(e) => e.stopPropagation()} className="looped-modal" style={{ background: 'rgba(255,251,246,.92)', border: '1px solid rgba(255,255,255,.9)', backdropFilter: 'blur(20px)', borderRadius: 22, animation: 'loopPop .35s ease' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 16 }}>
          ...
          <PlaceAutocomplete value={composer.cPlace} onChange={composer.setCPlace} placeholder="where?" style={textInputStyle} />
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <input
              ...
              min={composer.cTime || undefined}
              disabled={composer.cNoEndTime}
              onChange={composer.setCEndTime}
              style={{ ...textInputStyle, flex: 1, opacity: composer.cNoEndTime ? 0.5 : 1 }}
            />
          </div>
          <label style={{ display: 'flex', alignItems: 'center', gap: 6, font: '600 12.5px Karla,sans-serif', color: 'rgba(58,44,40,.7)', cursor: 'pointer' }}>
            <input type="checkbox" checked={composer.cNoEndTime} onChange={composer.toggleNoEndTime} />
            no end time
          </label>
          <select value={composer.cSpots} onChange={composer.setCSpots} style={selectStyle}>
